...
import { ArrowLeft, FilePlus, Landmark, Sun, Moon } from "lucide-react";
...
const NewApplication = () => {
  ...
  const { submissions, loading, addSubmission, deleteSubmission } = useFormSubmissions("नवीन अर्ज");
  ...
  const handleSaveAndPrint = async () => {
    ...
    setSaving(true);
    try {
      const formData = { mobile, aadhaar, address, applicationType, taluka, district, place };
      const saved = await addSubmission(name, formData);
      if (!saved) { setSaving(false); return; }
    } finally {
      setSaving(false);
    }
    setPrintData({ name, aadhaar, mobile, address, applicationType, taluka, district, place });
    setTimeout(() => {
      window.print();
      setName(""); setAadhaar(""); setMobile(""); setAddress(""); setApplicationType("");
    }, 300);
  };
  ...
  const p = printData || { name, aadhaar, mobile, address, applicationType, taluka, district, place };
  ...
  return (
    <div className="dash-root">
        ...
        {!showForm ? (
          <div style={{ display: "flex", justifyContent: "center", paddingTop: 20 }}>
            <button className="dash-card hamipatra-hero-card" style={{ maxWidth: 240, padding: "32px 24px 24px", animationDelay: "0s" }} onClick={() => setShowForm(true)}>
              ...
              <div className="dash-card-icon" style={{ background: "linear-gradient(135deg, #EDE9FE, #DDD6FE)", width: 64, height: 64 }}><FilePlus size={30} color="#7C3AED" strokeWidth={1.8} /></div>
              <span className="dash-card-label" style={{ fontSize: 14 }}>नवीन अर्ज (New Application)</span>
              <span style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", marginTop: 4 }}>फॉर्म भरण्यासाठी क्लिक करा →</span>
            </button>
          </div>
        ) : (
          <div className="form-container">
            <div className="form-header" style={{ background: themeGradient }}><h1 className="form-heading">नवीन अर्ज (New Application)</h1><p className="form-subheading">नवीन अर्जासाठी माहिती भरा</p></div>
            <div className="form-body">
              ...
              <div className="input-group"><label>अर्जाचा प्रकार</label><input type="text" value={applicationType} onChange={(e) => setApplicationType(e.target.value)} placeholder="उदा. लाडकी बहिण योजना, शेतकरी योजना" /></div>
      ...
      <div className="no-print">
        <SubmissionsList submissions={submissions} loading={loading} onDelete={deleteSubmission} onPrint={handlePrintRecord} columns={[{ key: "mobile", label: "मोबाईल" }, { key: "applicationType", label: "प्रकार" }]} />
      </div>
      ...
      <div className="print-only a4-page">
        <h2 className="print-title">नवीन अर्ज (New Application)</h2>
        ...
        <div className="print-row"><span className="print-label">अर्जाचा प्रकार :</span><span className="print-value-underline">{p.applicationType || "________________________"}</span></div>
