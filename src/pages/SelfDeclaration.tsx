...
const SelfDeclaration = () => {
  ...
  const { submissions, loading, addSubmission, deleteSubmission } = useFormSubmissions("स्वयंघोषणापत्र");
  ...
  const handleSaveAndPrint = async () => {
    ...
    setSaving(true);
    try {
      const formData = { mobile, aadhaar, address, purpose, taluka, district, place };
      const saved = await addSubmission(name, formData);
      if (!saved) { setSaving(false); return; }
    } finally {
      setSaving(false);
    }
    setPrintData({ name, aadhaar, mobile, address, purpose, taluka, district, place });
    setTimeout(() => {
      window.print();
  ...
  const p = printData || { name, aadhaar, mobile, address, purpose, taluka, district, place };
  ...
  return (
    <div className="dash-root">
        ...
        {!showForm ? (
          <div style={{ display: "flex", justifyContent: "center", paddingTop: 20 }}>
            <button className="dash-card hamipatra-hero-card" style={{ maxWidth: 240, padding: "32px 24px 24px", animationDelay: "0s" }} onClick={() => setShowForm(true)}>
              ...
              <div className="dash-card-icon" style={{ background: "linear-gradient(135deg, #D1FAE5, #A7F3D0)", width: 64, height: 64 }}><Shield size={30} color="#059669" strokeWidth={1.8} /></div>
          ...
          <div className="form-container">
            <div className="form-header" style={{ background: themeGradient }}><h1 className="form-heading">स्वयंघोषणापत्र (Self Declaration)</h1><p className="form-subheading">स्वयंघोषणापत्र माहिती भरा</p></div>
            <div className="form-body">
              ...
              <div className="input-group"><label>घोषणेचा उद्देश</label><input type="text" value={purpose} onChange={(e) => setPurpose(e.target.value)} placeholder="उद्देश लिहा" /></div>
      ...
      <div className="no-print">
        <SubmissionsList submissions={submissions} loading={loading} onDelete={deleteSubmission} onPrint={handlePrintRecord} columns={[{ key: "mobile", label: "मोबाईल" }, { key: "purpose", label: "उद्देश" }]} />
      </div>
      ...
      <div className="print-only a4-page">
        ...
        <div className="print-row"><span className="print-label">घोषणेचा उद्देश :</span><span className="print-value-underline">{p.purpose || "________________________"}</span></div>
